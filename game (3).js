...
let myFleet = [];             // [{id,size,cells:[{x,y}]}] — only ships actually placed
...
const views = ["lobby", "rules", "waiting", "queue", "place", "battle", "over"];
...
$("#btnRulesCreate").addEventListener("click", async () => {
  $("#rulesError").textContent = "";
  $("#btnRulesCreate").disabled = true;
  ...
  try {
    ...
    $("#waitingSub").textContent = isPublic
      ? "Public room — share the code, or just wait: Quick Match can pair someone straight in."
      : "Send this code to your opponent. Waiting for them to join…";
    showView("waiting");
    subscribeToGame(game.id);
  } catch (err) {
    $("#rulesError").textContent = err.message || "Couldn't create a room.";
  } finally {
    $("#btnRulesCreate").disabled = false;
  }
});
...
$("#btnQuickMatch").addEventListener("click", async () => {
  ...
  try {
    ...
    $("#lobbyError").textContent = err.message || "Couldn't start matchmaking.";
  } finally {
...
function subscribeToQueue() {
  ...
  queueChannel = sb
    ...
    .on(
      ...
      async (payload) => {
        const gameId = payload.new.matched_game_id;
        if (!gameId) return;
        queueChannel?.unsubscribe();
        const { data, error } = await sb.from("games").select("*").eq("id", gameId).single();
        if (error) {
          $("#lobbyError").textContent = "Matched, but couldn't load the game — try Quick match again.";
          showView("lobby");
          return;
        }
        game = data;
...
// The ship definition still waiting to be placed — the first one in
// FLEET_DEF order that isn't already in myFleet. Computed fresh each
// time so removing an earlier ship correctly re-opens its slot.
function nextShipDef() {
  return FLEET_DEF.find((def) => !myFleet.some((s) => s.id === def.id));
}
...
function buildPlaceGrid() {
  ...
  for (let y = 0; y < GRID_SIZE; y++) {
    for (let x = 0; x < GRID_SIZE; x++) {
      ...
      cell.addEventListener("click", () => onPlaceGridClick(x, y));
...
function onPlaceGridClick(x, y) {
  const existing = myFleet.find((s) => s.cells.some((c) => c.x === x && c.y === y));
  if (existing) {
    removeShip(existing.id);
  } else {
    placeShipAt(x, y);
  }
}
...
function removeShip(shipId) {
  const ship = myFleet.find((s) => s.id === shipId);
  if (!ship) return;
  ...
  myFleet = myFleet.filter((s) => s.id !== shipId);
  clearPreview();
  renderFleetList();
  updatePlaceHint();
...
function renderFleetList() {
  ...
  FLEET_DEF.forEach((def) => {
    ...
    } else if (activeDef && activeDef.id === def.id) {
...
function buildBattleGrids() {
  ...
  enemy.innerHTML = "";
  enemy.style.gridTemplateColumns = `repeat(${GRID_SIZE}, 1fr)`;
  for (let y = 0; y < GRID_SIZE; y++) {
    for (let x = 0; x < GRID_SIZE; x++) {
      const cell = document.createElement("div");
      cell.className = "cell";
  ...
  const own = $("#ownGrid");
  own.innerHTML = "";
  own.style.gridTemplateColumns = `repeat(${GRID_SIZE}, 1fr)`;
  for (let y = 0; y < GRID_SIZE; y++) {
    for (let x = 0; x < GRID_SIZE; x++) {
      const cell = document.createElement("div");
      cell.className = "cell";
...
// ------------------------------------------------------------
// Chat — only active when this game's settings enable it
// ------------------------------------------------------------
function setupChatForGame() {
...
function renderChatMessage(msg) {
  ...
  div.className = "chat-msg " + (msg.sender_id === userId ? "mine" : "theirs");
...
$("#chatForm").addEventListener("submit", async (e) => {
  ...
  input.value = "";
  try {
    const { error } = await sb.rpc("send_message", { p_game_id: game.id, p_body: text });
    if (error) throw error;
  } catch (err) {
    console.error(err);
  }
